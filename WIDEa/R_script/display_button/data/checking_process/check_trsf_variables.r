...
f_check_trsf_variables <- function (s_data_type = "normal", df_all, o_parameter) {
	if (s_data_type == "normal") {
		...
		v_message <- unlist(lapply(v_num, function(i) {
			...
			eval(parse(text = paste0("i_num <- length(isolate(o_parameter$", v_var_letter[i], "))"))) 
		...
		if (length(v_message_out) == 0) {
			...
			if (!is.na(isolate(o_parameter$f))) {
				eval(parse(text = paste0("x", 1:length(isolate(o_parameter$x)), " <- as.vector(df_all[, \"", isolate(o_parameter$x), "\"])")))
			...
			if (!is.na(isolate(o_parameter$g))) {
				eval(parse(text = paste0("y", 1:length(isolate(o_parameter$y)), " <- as.vector(df_all[, \"", isolate(o_parameter$y), "\"])")))
			...
			if (!is.na(isolate(o_parameter$h))) {
				eval(parse(text = paste0("z", 1:length(isolate(o_parameter$z)), " <- as.vector(df_all[, \"", isolate(o_parameter$z), "\"])")))
