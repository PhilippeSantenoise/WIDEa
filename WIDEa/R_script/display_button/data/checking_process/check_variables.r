...
#               (step 2) (1) ID variable has missing value(s) ?
#                        (2) Is the type/number of variables correct ? 
#                        (3) Are there any fields where variable(s) have no value ? 
...
#               (step 5) Are (".row_num.", ".f.", ".g.", ".h.") variables in loaded data (df_all input) ? 
#
...
#               The step 5 only concerns the normal/ir data type.  
# 
...
#                        Steps 2-1 and 2-2 are applied on loaded data (not sub-data).
...
# b_check_plus: boolean value is true if the data type is "normal", the plot type is "plot" and the model process is "none", false else.
# e_data: environment including loaded data and sub-data (if created)
	...
	if (s_data_type == "normal") {
		...
		if (length(v_pos) > 0) {
			...
			if (b_check_plus) {
				...
				v_pos <- c()
				if (length(v_pos_1) > 0) {eval(parse(text = paste0("v_pos <- c(v_pos, ", ifelse(length(v_pos_1) == length(o_parameter$x) & length(v_pos_1) == length(o_parameter$y), "NA", "v_pos_1"), ")")))}
				if (length(v_pos_2) > 0) {eval(parse(text = paste0("v_pos <- c(v_pos, ", ifelse(length(v_pos_2) == length(o_parameter$x) & length(v_pos_2) == length(o_parameter$z), "NA", "v_pos_2"), ")")))}
				if (length(v_pos_3) > 0) {eval(parse(text = paste0("v_pos <- c(v_pos, ", ifelse(length(v_pos_3) == length(o_parameter$y) & length(v_pos_3) == length(o_parameter$z), "NA", "v_pos_3"), ")")))}
			...
			if (length(s_e_message) == 0) {
				...
				if (isolate(o_parameter$plot_type) == "plot") {
					if (isolate(o_parameter$dim_num) == "3d") {
						...
						if (isolate(o_parameter$model) == "none") {
							...
							if (length(which(!is.na(o_parameter$ref))) > 0 & length(which(!is.na(o_parameter$wres_group))) > 0) {
								...
								eval(parse(text = paste0("v_cond <- c(", paste(paste0("is.character(e_data$all$", isolate(o_parameter$wres_group), ") | (is.numeric(e_data$all$", isolate(o_parameter$wres_group), ") & length(grep(\"[.]\", e_data$all$", isolate(o_parameter$wres_group), ")) == 0)"), collapse = ", "), ", ", paste(paste0("is.numeric(e_data$all$", isolate(o_parameter$x), ")"), collapse = ", "), ", is.numeric(e_data$all$", isolate(o_parameter$y), "))")))
								...
								eval(parse(text = paste0("v_cond <- c(", paste(paste0("is.numeric(e_data$all$", isolate(o_parameter$x), ")"), collapse = ", "), ", is.numeric(e_data$all$", isolate(o_parameter$y), "))")))
					...
					if (length(which(!is.na(isolate(o_parameter$y)))) < 2) { # return a message if the number of Y variables is inferior to 2
						...
						eval(parse(text = paste0("v_cond <- c(", paste(paste0("is.numeric(e_data$all$", isolate(o_parameter$y), ")"), collapse = ", "), ")")))
