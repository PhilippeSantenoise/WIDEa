...
# df_all: data saved in the e_data environment (e_data$all or e_data$sub if sub-data is created)
# v_sub_row: row number corresponding to sub-data (only used if no ID variable is assocciated to flag data
...
# l_graph_opt: list of graph option (color, opacity, point type/size) created by the f_create_graph_opt_vector function. This input is used for the ir data type (process 2).
	...
	if (s_data_type == "normal") {
		if (i_proc_num == 1) {
					...
					if (isolate(o_parameter$plot_type) == "plot") {
						if (isolate(o_parameter$dim_num) == "2d") {
							if (isolate(o_parameter$model) == "none") {
								...
								eval(parse(text = paste0("df_all <- df_all[", paste(paste0("!is.na(df_all[, \"", c(isolate(o_parameter$x), isolate(o_parameter$y)), "\"])"), collapse = " & "), ",]")))
							}
							else {
								...
								if (length(which(!is.na(isolate(o_parameter$ref)))) > 0) {eval(parse(text = paste0("df_all <- df_all[", paste(paste0("!is.na(df_all[, \"", isolate(o_parameter$ref), "\"])"), collapse = " & "), ",]")))}
								if (length(which(!is.na(isolate(o_parameter$wres_group)))) > 0) {eval(parse(text = paste0("df_all <- df_all[", paste(paste0("!is.na(df_all[, \"", isolate(o_parameter$wres_group), "\"])"), collapse = " & "), ",]")))}
							}
						}
						else {
							if (is.na(isolate(o_parameter$id))) {
							...
							eval(parse(text = paste0("df_all <- df_all[", paste(paste0("!is.na(df_all[, \"", c(isolate(o_parameter$x), isolate(o_parameter$y), isolate(o_parameter$z)), "\"])"), collapse = " & "), ",]")))
						}
					}
					else if (isolate(o_parameter$plot_type) == "boxplot") {
						eval(parse(text = paste0("df_all <- df_all[!is.na(df_all[, isolate(o_parameter$x)]) & ", paste(paste0("!is.na(df_all[, \"", isolate(o_parameter$y), "\"])"), collapse = " & "), ",]")))
					}
					else {
						eval(parse(text = paste0("df_all <- df_all[", paste(paste0("!is.na(df_all[, \"", isolate(o_parameter$x), "\"])"), collapse = " & "), ",]")))
					}
				}
			}
			else {
			...
			if (sum(lengths(l_fun_val)) > 0) {	
				...
				if (length(v_pos) > 0) {
					eval(parse(text = paste0("v_name <- c(", paste(paste0("isolate(o_parameter$", c("x", "y", "z")[v_pos], ")"), collapse = ", "), ")")))
				}
				else {
			...
			if (length(v_name) > 0){
				...
				if (length(v_pos) > 0) {eval(parse(text = paste0("df_all <- data.frame(", paste(paste0("\".", v_fun_letter[v_pos], ".\" = l_fun_val$", v_fun_letter[v_pos]), collapse = ", "), ")"))) }
			}
			...
			if (length(v_pos) < 2) {
				...
				if (length(v_pos) > 0) {
					...
					if (!is.na(isolate(o_parameter$id))) {
						...
						eval(parse(text = paste0("v_id_0 <- ", ifelse(isolate(o_parameter$data_name) == "sub", "v_sub_row", "1:dim(df_all)[1]"))))
						v_pos_1 <- c()
						if (!is.na(isolate(o_parameter$group))) {eval(parse(text = paste0("v_pos_1 <- ", ifelse(isolate(o_parameter$data_name) == "sub", "v_sub_row[which(is.na(df_all[, isolate(o_parameter$group)]))]", "which(is.na(df_all[, isolate(o_parameter$group)]))"))))}
					}
