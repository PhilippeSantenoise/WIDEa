...
#
#
# Description : function used to prepare data from data (raw/flag) saved in the e_data environment.
#               All data created from this function are saved in the o_plot reactive value ("data" for 
#               raw data, "data_qc1" and "data_qc2" for flag data with a qc = 1 or 2 value). 
#               The data preparation is performed in two (normal/ir data type) or three (temporal data
#               type) processes.
#               
#               (process 1) - create data (raw and flag by qc value)
#                           - remove qc = 2 flag data in raw data
#                           - check if the row number of raw data is superior to 0.
#                           - create a binary vector used to inform if flags (by qc values) exist
#                           (ir) - create data corresponding to isolated points
#                                - check if code variable  range in (spectra) raw data is the same as 
#                                  code-frequency data
#                                - check if the spectra length (without missing values) is identical
#               (process 2) (normal/temporal) replace values of (X, Y, Z) variables by values of 
#                                             variables transformed by (f, g, h) functions
#                           (ir) transform data created in the process 1 (transposed data, data including  
#                                id/group informations).
#               (process 3) (temporal) create data corresponding to isolated points 
#
...
# i_proc_num: process number (only used for the normal data type: value = 1, 2)
# df_all: data saved in the e_data environment (e_data$all)
# df_code_freq: data saved in the e_data environment (e_data$code_freq)
# v_code_range: code range saved in the o_data_info reactive value (see R script "WIDEa_launcher"). This input only concerns the ir data type.
# df_flag: data saved in the e_data environment (e_data$flag)
# s_flag_name: flag data name saved in the o_flag reactive value (see R script "WIDEa_launcher"). This input only concerns the normal data type.
# i_flag_msg: binary value associated to a warning message (= Flag tab is disabled when a f, g and/or h function is added 
#             or some options are selected in Statistics tab).
#             If value = 1, then the warning message is not returned by the function (used not to display again the same
#             message for the current data/plot type selection). 
#             This input only concerns the normal data type.
# df_previous_flag: data saved in the e_previous_flag environment (e_previous_flag$data)
# l_qc: list including qc = 1 and 2 data. This input is only used for the ir data type (data created for the process 1 and required for the process 2).
# l_id: list including id data (id = row number if the ID variable is not informed). This input is only used for the ir data type (data created for the 
#       process 1 and required for the process 2).
# l_fun_val: list including transformed variable values. This input is only used for the normal/temporal data type (process 2).
# o_parameter: reactive value from the R script "WIDEa_launcher"
# v_color: color palette created by the f_create_col_palette function. This input is used for the ir data type (process 2).

# Output:
# -------
# return a list of data generated by different processes

f_prepare_data <- function (s_data_type = "normal", i_proc_num = 1, df_all, df_code_freq = NULL, v_code_range = NULL, df_flag = NULL, s_flag_name = NULL, i_flag_msg = 0, df_previous_flag = NULL, l_qc = NULL, l_id = NULL, l_fun_val = NULL, o_parameter, v_color = NULL) {
	if (s_data_type == "normal") {
		if (i_proc_num == 1) {
			v_cond <- c(0, i_flag_msg) # qc2 & i_flag_msg
			...
			if (isolate(o_parameter$plot_type) != "barplot") {
				if (!is.null(df_flag)) {
					...
					if (length(which(isolate(o_parameter$model) %in% c("calib", "valid"))) > 0) {
						if (length(which(!is.na(isolate(o_parameter$ref)))) > 0) {
							v_var_add <- c(v_var_add, isolate(o_parameter$ref))
						}
						
						if (length(which(!is.na(isolate(o_parameter$wres_group)))) > 0) {
							v_var_add <- c(v_var_add, isolate(o_parameter$wres_group))
						}
					}
					
					if (!is.na(isolate(o_parameter$group))) {
						v_var_add <- c(v_var_add, isolate(o_parameter$group))
					}
					...
					if (length(v_pos) > 0) {
						...
						if (length(which(v_sum > 0)) > 0) {
							v_id <- as.vector(df_flag[which(v_sum > 0), 1])
							...
							if (substr(s_flag_name, nchar(s_flag_name) - 9, nchar(s_flag_name) - 4) == "withID") {
								...
								df_all[which(df_all[, names(df_flag)[1]] %in% v_id), v_var_qc2][df_flag[which(v_sum > 0), v_var_qc2] == 1] <- NA
							}
							else {
								df_qc2 <- as.data.frame(df_all[v_id, v_var])
								df_all[v_id, v_var_qc2][df_flag[which(v_sum > 0), v_var_qc2] == 1] <- NA
							}
							...
							if (length(v_var) == 1) {
								...
								eval(parse(text = paste0("df_qc2 <- df_qc2[", paste(paste0("!is.na(df_qc2[, \"", v_var, "\"])"), collapse = " & "), ",]")))
							}
							...
							if (is.na(isolate(o_parameter$f)) & is.na(isolate(o_parameter$g)) & is.na(isolate(o_parameter$h))) {
								if (isolate(o_parameter$plot_type) == "plot") {
									if (dim(df_qc2)[1] > 0) {
										v_cond[1] <- 1
										df_qc2_out <- df_qc2
									...
									if (dim(df_qc2)[1] > 0) {
										...
										s_w_message <- "Data with a qc = 2 flag (no matching with the current selection) are only displayed for a 2D/3D plot (model: none) without (f, g, h) functions associated to (X, Y, Z) variables"
									}
								}
							}
							else {
								if (dim(df_qc2)[1] > 0) {
									if (i_flag_msg == 0) {
										s_w_message <- paste0("Data with a qc = 2 flag (corresponding to ", dim(df_qc2)[1], " row", ifelse(dim(df_qc2)[1] > 1, "s", ""), ") are only displayed for a 2D/3D plot with no selected model and no function associated to (X, Y, Z) variables<br/>The flag tab is disabled because a function (f, g, h) is enabled or one of these options is selected in the Statistics tab: linear regression, confidence ellipsoid, centroid")
										v_cond[2] <- 1
									}
									else {
										s_w_message <- paste0("Data with a qc = 2 flag (corresponding to ", dim(df_qc2)[1], " row", ifelse(dim(df_qc2)[1] > 1, "s", ""), ") are only displayed for a 2D/3D plot with no selected model and no function associated to (X, Y, Z) variables")
									...
									if (i_flag_msg == 0) {
										s_w_message <- "Data with a qc = 2 flag (no matching with the current selection) are only displayed for a 2D/3D plot (model: none) without (f, g, h) functions associated to (X, Y, Z) variables<br/>The flag tab is disabled because a function (f, g, h) is enabled or one of these options is selected in the Statistics tab: linear regression, confidence ellipsoid, centroid"
										v_cond[2] <- 1
									}
									else {
										s_w_message <- "Data with a qc = 2 flag (no matching with the current selection) are only displayed for a 2D/3D plot (model: none) without (f, g, h) functions associated to (X, Y, Z) variables"
					...
					if (!is.na(isolate(o_parameter$f)) | !is.na(isolate(o_parameter$g)) | !is.na(isolate(o_parameter$h))) {
						if (i_flag_msg == 0) {
							s_w_message <- "The flag tab is disabled because a function (f, g, h) is enabled or one of these options is selected in the Statistics tab: linear regression, confidence ellipsoid, centroid"
							v_cond[2] <- 1
						}
					}
				}
				...
				if (!is.na(isolate(o_parameter$group)) & length(which(isolate(o_parameter$model) == "calib")) > 0) {
					if (length(which(is.na(df_all[, isolate(o_parameter$group)]))) > 0) {
						s_w_message <- ifelse(length(s_w_message) == 0, "Missing value(s) detected for the Group variable", paste0(s_w_message, "<br/>Missing value(s) detected for the Group variable"))
					}
				}
					...
					if (isolate(o_parameter$plot_type) == "plot") {
						if (isolate(o_parameter$dim_num) == "2d") {
							if (isolate(o_parameter$model) == "none") {
								...
								eval(parse(text = paste0("df_all <- df_all[", paste(paste0("!is.na(df_all[, \"", isolate(o_parameter$x), "\"])"), collapse = " & "), " & !is.na(df_all[, isolate(o_parameter$y)]),]")))
							
								if (length(which(!is.na(isolate(o_parameter$ref)))) > 0) {
									eval(parse(text = paste0("df_all <- df_all[", paste(paste0("!is.na(df_all[, \"", isolate(o_parameter$ref), "\"])"), collapse = " & "), ",]")))
								}
								
								if (length(which(!is.na(isolate(o_parameter$wres_group)))) > 0) {
									eval(parse(text = paste0("df_all <- df_all[", paste(paste0("!is.na(df_all[, \"", isolate(o_parameter$wres_group), "\"])"), collapse = " & "), ",]")))
								}
							}
						}
						else {
			...
			if (dim(df_all)[1] == 0) {
				s_e_message <- "All row values are marked with a qc = 2 flag when the selected variables are combined"
			}
			
			return(list(df_all, df_qc2_out, v_cond, s_e_message, s_w_message))
		}
		else { # process = 2
			if (sum(lengths(l_fun_val)) > 0) {
				if (length(l_fun_val$f) > 0) {
					df_all[, isolate(o_parameter$x)] <- l_fun_val$f
				}
				
				if (length(l_fun_val$g) > 0) {
					df_all[, isolate(o_parameter$y)] <- l_fun_val$g
				}
				
				if (length(l_fun_val$h) > 0) {
					df_all[, isolate(o_parameter$z)] <- l_fun_val$h
				}
			}
			...
			v_name <- c(v_name, isolate(o_parameter$id), isolate(o_parameter$x), isolate(o_parameter$y), isolate(o_parameter$z), isolate(o_parameter$group))
					...
					if (!is.na(isolate(o_parameter$g))) {
						...
						if (length(v_var_qc2) > 0) {
							...
							for (i in 1:length(v_var_qc2)) {
								...
								eval(parse(text = paste0("v_pos <- c(", paste(paste0("min(which(df_all$", isolate(o_parameter$x), "_trf >= ", as.vector(df_flag_select$date_start_trf), ")):max(which(df_all$", isolate(o_parameter$x), "_trf <= ", as.vector(df_flag_select$date_end_trf), "))"), collapse = ", "), ")")))
								eval(parse(text = paste0("v_qc <- c(", paste(paste0("rep(", df_flag_select$qc, ", length(min(which(df_all$", isolate(o_parameter$x), "_trf >= ", as.vector(df_flag_select$date_start_trf), ")):max(which(df_all$", isolate(o_parameter$x), "_trf <= ", as.vector(df_flag_select$date_end_trf), "))))"), collapse = ", "), ")"))) 
								df_qc[v_pos, paste0("qc_", v_var_qc2[i])] <- v_qc
							}
							
							df_all[, -c(1, 2)][df_qc[, -c(1, 2)] == 2] <- NA
							
							eval(parse(text = paste0("v_cond_NA <- c(", paste(paste0("length(which(is.na(df_all[, \"", isolate(o_parameter$y), "\"]))) == dim(df_all)[1]"), collapse = ", "), ")")))
						...
						for (i in 1:length(v_var_all)) {
							...
							eval(parse(text = paste0("v_pos <- c(", paste(paste0("min(which(df_all$", isolate(o_parameter$x), "_trf >= ", as.vector(df_flag_select$date_start_trf), ")):max(which(df_all$", isolate(o_parameter$x), "_trf <= ", as.vector(df_flag_select$date_end_trf), "))"), collapse = ", "), ")")))
							eval(parse(text = paste0("v_qc <- c(", paste(paste0("rep(", df_flag_select$qc, ", length(min(which(df_all$", isolate(o_parameter$x), "_trf >= ", as.vector(df_flag_select$date_start_trf), ")):max(which(df_all$", isolate(o_parameter$x), "_trf <= ", as.vector(df_flag_select$date_end_trf), "))))"), collapse = ", "), ")"))) 
							eval(parse(text = paste0("v_comment <- c(", paste(paste0("rep(\"", df_flag_select$comment, "\", length(min(which(df_all$", isolate(o_parameter$x), "_trf >= ", as.vector(df_flag_select$date_start_trf), ")):max(which(df_all$", isolate(o_parameter$x), "_trf <= ", as.vector(df_flag_select$date_end_trf), "))))"), collapse = ", "), ")"))) 
						...
						if (length(v_pos) > 0) { 
							...
							if (length(v_leg_name_select) > 0) {
								v_leg_name_qc <- v_leg_name_qc[which(v_leg_name_qc %in% v_leg_name_select)]
							}
		...
		if (i_proc_num == 1) {
			...
			if (length(v_pos) < 2) {
				...
				if (v_pos > 1) {
					v_code <- v_code[-c(1:(v_pos - 1))]
				}
				
				v_pos <- which(v_code == v_code_range[2])
				
				if (v_pos < length(v_code)) {
					v_code <- v_code[-c((v_pos + 1):length(v_code))]
				}
				...
				if (length(v_pos) > 0) {
					...
					if (length(which(v_diff == 2)) > 0) {
						v_pt_pos <- v_pos[which(v_diff == 2)] + 1
					}
					
					v_pos <- which(!v_code %in% names(df_all))
					
					if (length(v_pos) > 0) {
						eval(parse(text = paste(paste0("df_all$", v_code[v_pos], " <- NA"), collapse = "; ")))
					}
					...
					if (!is.na(isolate(o_parameter$id))) {
						...
						v_id_0 <- 1:dim(df_all)[1]
						v_pos_1 <- c()
						
						if (!is.na(isolate(o_parameter$group))) {
							v_pos_1 <- which(is.na(df_all[, isolate(o_parameter$group)]))
						}
					}
							...
							if (!is.na(isolate(o_parameter$id))) {
								...
								if (length(v_pos_1) > 0) {
									...
									if (length(v_pos_2) < length(v_id_2) & length(v_pos_2) > 0) {
										...
										df_qc2 <- df_all[v_id_2,]
										v_cond[3] <- 1
										df_all <- df_all[-c(v_pos_1, v_id_2),]
										v_id_0 <- v_id_0[-c(v_pos_1, v_id_2)]
									}
								}
								else {
									df_qc2 <- df_all[v_id_2,]
									v_cond[3] <- 1
									df_all <- df_all[-v_id_2,]
									v_id_0 <- v_id_0[-v_id_2]
								}
							}
						}
						else {
			...
			df_id_group <- data.frame("id" = rep(NA, length(v_name)), "group" = rep(NA, length(v_name)), "show" = rep(F, length(v_name)), "color" = rep(NA, length(v_name))) 
			...
			if (is.na(isolate(o_parameter$group))) {
				...
				eval(parse(text = paste0("df_id_group$color <- c(", paste(paste0("rep(\"", v_color, "\", ", lengths(l_name), ")"), collapse = ", "), ")")))
