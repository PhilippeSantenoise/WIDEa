...
# (o_click_button, o_plot, o_parameter, o_picture_info, o_label_text): reactive values from the R script "WIDEa_launcher" 
	...
	if (s_data_type == "normal") {
		if (isolate(o_parameter$plot_type) == "plot") {
			...
			if (isolate(o_parameter$model) == "none") {
				...
				eval(parse(text = paste0("v_round_x <- round(df_all[, \"", s_var_x, "\"], digits = i_dec_num)")))
				eval(parse(text = paste0("v_round_y <- round(df_all[, \"", s_var_y, "\"], digits = i_dec_num)")))
				...
				if (isolate(o_parameter$dim_num) == "2d") {
					...
					if (!is.na(isolate(o_parameter$group))) {
						...
						eval(parse(text = paste0("v_x <- as.vector(df_all[, \"", s_var_x, "\"])")))
						eval(parse(text = paste0("v_y <- as.vector(df_all[, \"", s_var_y, "\"])")))
					...
					eval(parse(text = paste0("v_round_z <- round(df_all[, \"", s_var_z, "\"], digits = i_dec_num)")))
					...
					if (!is.na(isolate(o_parameter$group))) {
						...
						eval(parse(text = paste0("v_x <- as.vector(df_all[, \"", s_var_x, "\"])")))
						eval(parse(text = paste0("v_y <- as.vector(df_all[, \"", s_var_y, "\"])")))
						eval(parse(text = paste0("v_z <- as.vector(df_all[, \"", s_var_z, "\"])")))
			...
			if (!is.na(isolate(o_parameter$group))) {
				...
				v_group <- v_group[order(v_group)]
				v_visible <- df_click_legend[which(df_click_legend$name %in% v_group), "statut"]
				eval(parse(text = paste0("l_y <- list(", paste(paste0("as.vector(df_all[which(df_all$x == \"", v_group, "\"), \"y\"])"), collapse = ", "), ")")))
