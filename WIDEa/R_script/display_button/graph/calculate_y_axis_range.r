...
#
#
# Description : the function is associated to the "Y scale" parameter in the Graphic tab (top panel) and 
#               allows to calculate the Y axis range when the auto/manual option is selected.   
#
...
# Input:
# ------
# s_data_type: data type (2 values: "temporal", "ir") 
# s_scale: options of the "Y scale" parameter ("auto" or "manual")
# df_all_orig: data saved in the e_data environment ("all" or "sub" if sub-data is created) 
# (o_parameter, o_zoom, o_plot, o_cond): reactive values from the R script "WIDEa_launcher"
# df_click_legend: legend item informations (name and status)
# n_frac: fraction of the graph height (value between 0 and 0.1)
# v_range: vector composed of values filled in "min" and "max" inputs of the manual otpion.

# Output:
# -------
# return a list with the calculated Y range (two ranges returned for the auto option: range with/without fraction) and a warning message (if a problem occurs) 
