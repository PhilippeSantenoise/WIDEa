...
f_add_dens_curve <- function (ply_1, v_group = "all", df_click_legend, o_plot, o_parameter, i_w_message) {
	s_w_message <- NULL
	s_e_message <- NULL
	...
	if (length(v_group) > 0) {
		...
		if (!is.na(isolate(o_parameter$group))) {
			...
			eval(parse(text = paste(paste0("l_density_", 1:length(v_group), " <- density(df_all[df_all[, isolate(o_parameter$group)] == \"", v_group, "\", isolate(o_parameter$x)])"), collapse = "; ")))
			...
			df_group <- df_all[df_all[, isolate(o_parameter$group)] %in% v_group,]
			n_bartlett_pval <- eval(parse(text = paste0("tryCatch({suppressWarnings(bartlett.test(", isolate(o_parameter$x), " ~ ", isolate(o_parameter$group), ", data = df_group)$p.value)}, error = function(e) FALSE)"))) # Bartlett test
			s_test1 <- ifelse(is.numeric(n_bartlett_pval), paste0("<br>Bartlett: p-value ", f_numeric_trsf(n_bartlett_pval, b_pval = T)), "")
			n_levene1_pval <- eval(parse(text = paste0("tryCatch({suppressWarnings(leveneTest(", isolate(o_parameter$x), " ~ ", isolate(o_parameter$group), ", data = df_group, center = mean)$\"Pr(>F)\"[1])}, error = function(e) FALSE)"))) # Levene test (center mean)
			s_test2 <- ifelse(is.numeric(n_levene1_pval), paste0("<br>Levene (center = mean): p-value ", f_numeric_trsf(n_levene1_pval, b_pval = T)), "")
			n_levene2_pval <- eval(parse(text = paste0("tryCatch({suppressWarnings(leveneTest(", isolate(o_parameter$x), " ~ ", isolate(o_parameter$group), ", data = df_group, center = median)$\"Pr(>F)\"[1])}, error = function(e) FALSE)"))) # Levene test (center mean)
			...
			ply_1 <- add_trace(p = ply_1, x = l_density$x, y = l_density$y, name = "all (curve)", type = "scatter", mode = "lines", line = list(color = adjustcolor(isolate(o_plot$color), alpha.f = n_op + 0.3)), fill = "tozeroy", fillcolor = adjustcolor(isolate(o_plot$color), alpha.f = n_op), legendgroup = "all (curve)", hoverinfo = 'text+name', hoverlabel = list(bgcolor = adjustcolor(isolate(o_plot$color), alpha.f = 0.7)), text = s_text, showlegend = T, visible = eval(parse(text = v_visible)))
			
			# add mean/median:
			
			ply_1 <- add_trace(p = ply_1, x = rep(n_mean, 2), y = c(0, approx(l_density$x, l_density$y, xout = n_mean)$y), type = "scatter", mode = "lines", line = list(color = adjustcolor(isolate(o_plot$color), alpha.f = n_op + 0.3), dash = "dash"), legendgroup = "all (curve)", showlegend = F, visible = eval(parse(text = v_visible)))
			ply_1 <- add_trace(p = ply_1, x = rep(n_median, 2), y = c(0, approx(l_density$x, l_density$y, xout = n_median)$y), type = "scatter", mode = "lines", line = list(color = adjustcolor(isolate(o_plot$color), alpha.f = n_op + 0.3), dash = "dot"), legendgroup = "all (curve)", showlegend = F, visible = eval(parse(text = v_visible)))
		}
	}
	else {
		if (!is.na(isolate(o_parameter$group))) {
