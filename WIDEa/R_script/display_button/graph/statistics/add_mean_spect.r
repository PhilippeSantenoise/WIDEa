...
f_add_mean_spect <- function (ply_1, df_click_legend, o_plot, o_parameter) {
	...
	eval(parse(text = paste(paste0("ply_1 <- add_trace(p = ply_1, x = df_all$Frequency, y = l_var[[", 1:length(l_var), "]]$y, type = \"" , ifelse(isolate(o_parameter$webgl) == "yes", "scattergl", "scatter"), "\", mode = \"lines\", line = list(color = adjustcolor(\"", v_color, "\", alpha.f = 0.95), width = 3, dash = \"dash\"), name = \"" , unique(v_group), " (mean)\", hoverinfo = 'text+name', text = l_var[[", 1:length(l_var), "]]$info, legendgroup = \"" , unique(v_group), " (mean)\", showlegend = T, visible = ", v_visible, ")"), collapse = "; ")))
	...
	if (length(which(!is.na(isolate(o_plot$pt_pos)))) > 0) {
		...
		eval(parse(text = paste(paste0("ply_1 <- add_trace(p = ply_1, x = as.vector(df_all[isolate(o_plot$pt_pos), \"Frequency\"]), y = l_var[[", 1:length(l_var), "]]$y, type = \"" , ifelse(isolate(o_parameter$webgl) == "yes", "scattergl", "scatter"), "\", mode = \"markers\", marker = list(line = list(color = adjustcolor(\"", v_color, "\", alpha.f = 0.95), width = 3), color = adjustcolor(\"", v_color, "\", alpha.f = 0.5), size = 6), name = \"" , unique(v_group), " (mean)\", hoverinfo = 'text+name', text = l_var[[", 1:length(l_var), "]]$info, legendgroup = \"" , unique(v_group), " (mean)\", showlegend = F, visible = ", v_visible, ")"), collapse = "; ")))
	}
