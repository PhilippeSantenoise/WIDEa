...
f_add_lreg <- function (ply_1, v_group = "all", df_click_legend, o_plot, o_parameter, o_lreg_info, i_w_message, v_color) {
	s_w_message <- NULL
	s_e_message <- NULL
	...
	if (length(v_group) > 0) {
		...
		if (!is.na(isolate(o_parameter$group))) {
			...
			l_val <- lapply(v_group, function(x) {
				...
				n_r2 <- round(1 - sum((df_group$y - fitted(l_lreg))^2)/sum((df_group$y - mean(df_group$y))^2), digits = 2)
				
				if (isolate(o_parameter$model) == "none") {
					n_rmse <- sqrt(sum((df_group$y - fitted(l_lreg))^2) / length(df_group$y))
			...
			v_y <- n_intercept + n_slope * v_x
			s_text <- paste0(ifelse(is.na(isolate(o_parameter$g)), "y", "g(y)"), " = a + b.", ifelse(isolate(o_parameter$model) != "none", paste0("fit(", ifelse(is.na(isolate(o_parameter$g)), "y", "g(y)"), ")"), ifelse(is.na(isolate(o_parameter$f)), "x", "f(x)")), "<br>a = ", s_intercept, "<br>b = ", s_slope)
			n_r2 <- round(1 - sum((df_all$y - fitted(l_lreg))^2)/sum((df_all$y - mean(df_all$y))^2), digits = 2)
			
			if (isolate(o_parameter$model) == "none") {
				n_rmse <- sqrt(sum((df_all$y - fitted(l_lreg))^2) / length(df_all$y))
				s_rmse <- f_numeric_trsf(n_rmse, b_pval = F)
			}
			else {
				n_test1 <- f_ttest(l_lreg, 1, 0) # intercept = 0 t-test
			...
			ply_1 <- add_trace(p = ply_1, x = v_x, y = v_y, name = "all (lreg)", type = ifelse(isolate(o_parameter$webgl) == "yes", "scattergl", "scatter"), mode = "lines", line = list(color = adjustcolor(v_color, alpha.f = 1), width = 3, dash = "dash"), hoverinfo = 'text+name', text = s_text, showlegend = T, visible = v_visible)
