...
f_add_current_flag <- function (s_data_type = "normal", s_dim_num = "2d", l_temp_coord = NULL, o_parameter, o_plot = NULL, o_click_ev, i_flag_num, l_click_info = NULL) {
	...
	if (s_data_type == "normal") {
		if (s_dim_num == "2d") {
			...
			l_add_traces <- list(
				x = rep(o_click_ev[["x"]], 2), y = rep(o_click_ev[["y"]], 2), z = rep(o_click_ev[["z"]], 2), type = 'scatter3d', mode = 'markers', marker = list(line = list(color = "black", width = 2), color = adjustcolor("black", alpha.f = 0.5), size = 8), hoverinfo = 'text', hoverlabel = list(bgcolor = "black"), text = paste0("(flag ", i_flag_num, ")<br>x: ", round(o_click_ev[["x"]], digits = i_dec_num), "<br>y: ", round(o_click_ev[["y"]], digits = i_dec_num), "<br>z: ", round(o_click_ev[["z"]], digits = i_dec_num)), showlegend = F
			)
		}
	}
