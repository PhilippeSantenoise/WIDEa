...
#
#
# Description: (Server) Function used to load all data
#
# Creation date: September 2022
####################################################################################

# Function inputs
# s_id: action button input ID's used to load data (3 values: "load1_button", "load2_button", "load3_button")
# s_data_type: data type (3 values: "normal", "temporal", "ir")
# s_path: data path
# b_flag: boolean value used to load flag data when the "load1_button" action button is clicked
# s_model: model type (3 values: "none", "calib", "valid"). This input is only used when the "load3_button" action button is clicked
...
f_load_data <- function(s_id, s_data_type, s_path, b_flag = F, s_model = "none") {
	...
	if (s_id == "load1_button") {
		s_w_message <- NA
		df_1 <- tryCatch({suppressWarnings(fread(file = s_path, data.table = F))}, error = function(e) F) 
		df_flag <- data.frame()
		v_code_range <- c()
		b_special <- F
							...
							if (s_data_type == "normal") {
								if (b_cond_1 & b_cond_2) {
									...
									if (b_cond_2) {
										...
										df_flag <- fread(file = s_flag_path_2, data.table = F)
										...
										df_flag <- fread(file = s_flag_path_1, data.table = F)
										if (dim(df_flag)[1] == 0) {s_e_message <- "Flag data is empty"}
									}
								}
							}
							else if (s_data_type == "temporal") {
								df_flag <- fread(file = s_flag_path, data.table = F)
								if (dim(df_flag)[1] == 0) {s_e_message <- "Flag data is empty"}
							}
							else {
								if (b_cond_1 & b_cond_2) {
