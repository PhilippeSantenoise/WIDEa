...
#
#
# Description: (Server) Create UI's input inventories
#
# Creation date: April 2022
####################################################################################

# Function inputs
# s_by: order otpion (two values: "section" as default, "type")
# s_type: input type (four values: "selectize" as default, "radio_button", "check_box_group", "numeric")
# df_all: data frame saved in e_data environment (e_data$all)
# v_concat: vector of two binary values (given by o_cond reactive value: concat1, concat2) used to remove variables named ".concat1." and 
#           ".concat2." (respectively) from e_data$all
# i_concat: integer associated to o_cond$concat1 reactive value
# b_concat1: boolean value associated to concat1 check box
# b_concat2: boolean value associated to concat2 check box
# i_sub_data: binary value associated to sub-data (e_data$sub). 1 if sub-data is created.
# i_only_value: integer value used to return specific values of selectize inputs (values: 0, 1 or 2). O returns a list of all selectize input
#               choices. 1 only returns the vector of variable names available from df_all (e_data environment). 2 only returns the vector of 
#               unique ordered values associated to the variable selected from the "vname" selectize input (subdata creation section).
# s_sub_var_name: value returned by the "vname" selectize input (NULL as default value)
# s_sub_var_type: value returned by the "vtype" radio button input (2 values: "qualit", "quanti")
# s_data: data type (three values: "normal", "temporal", "ir")
# s_graph: graph type (six values: "plot_2d", "plot_3d", "histplot", "corplot", "boxplot", "barplot")
# s_model: model procedure (three values: none as default, "calib", "valid"). This option is only  
#          available if s_data is "normal" and s_graph is "plot_2d".
# b_model: boolean value associated to model radio button (T if a calibration/validation model is selected)
# b_display: boolean value associated to the display button (clicked = T; not clicked = F)
# b_load2: boolean value associated to the load2 button (clicked = T; not clicked = F)
# b_load3: boolean value associated to the load3 button (clicked = T; not clicked = F)
# o_selectize_input: reactive value with max item information for selectize inputs (ID: "var_x", "var_y")
...
# b_flag: boolean value associated to e_data$flag (T if flag data exist in e_data environment, F else)


# Create a list with all UI's input IDs ordered by panel section or type (selectize, numeric, text, action button, radio button, check box)
...
# Create a data frame with matching IDs between UI's input and o_parameter reactive value. Data are used to update inputs in the variable selection section (left panel) 
...
f_create_input_value_list <- function(s_type = "selectize", df_all = NULL, v_concat = rep(0, 2), i_sub_data = 0, i_only_value = 0, s_sub_var_name = NULL, s_sub_var_type = "qualit") {
	if (s_type == "selectize") {
		...
		if (i_only_value == 0) {
			eval(parse(text = paste0("l_id <- list(", paste(paste0("\"", c("vname", paste0("var_", c("id", "x", "y", "z", "group")), "ref", "wres_group"), "\" = 1"), collapse = ", "), ", \"date_format\" = 2, \"rel_symbol\" = 3, \"vvalue1\" = 4, \"edit_option\" = 5)")))
			eval(parse(text = paste0("l_value <- list(v_var_name, c(\"%Y%m%d\", \"%Y%m%d%H%M\", \"%d%m%Y\", \"%d%m%Y%H%M\"), ", ifelse(s_sub_var_type == "qualit", "c(\"%in%\", \"!%in%\")", "c(\"=\", \"!=\", \"<\", \">\", \"<=\", \">=\")"), ", v_value, c(\"label\", \"color/opacity\", \"point type/size\"))")))
...
# Create a list for radio button inputs with number used as a special enabling/desabling
f_create_input_sp_num_list <- function() {
	l_out <- list("id" = 1, "f_radio" = 2, "g_radio" = 2, "h_radio" = 2, "webgl" = 2, "vtype" = 3)
	return(l_out)
}

# Create a list for selectize input IDs ("var_x", "var_y") with new maximum item values to be replaced 
...
# Update lists of selectize input ID with the maximum item value (saved in o_selectize_input reactive value)
...
# Create a vector of input IDs associated to the variable selection section (left panel) 
# -> Input IDs are used to save the status (enabled/disabled) once the display button is clicked  
...
# Create a vector of input IDs associated to the Statistics tab
...
# Create a vector of input IDs enabled when a data/graph type and/or model procedure is selected
