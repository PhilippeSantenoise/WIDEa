...
#
#
# Description : (corplot, normal data type) the function allows to create a new graph in a modal dialog  
#               popup window when a click event is executed on a XY cell of the correlation matrix
#
...
f_build_modal_graph <- function (df_all,  i_num) {
	...
	if (b_cond) { # add linear regression to XY scatter-plot
		eval(parse(text = paste0("l_lreg <- lm(", s_y_var, " ~ ", s_x_var, ", data = df_all)")))
	...
	if (i_num == 0) {
		...
		if (b_cond) {
			ply_1_2 <- add_trace(p = ply_1_2, x = v_x, y = v_y, name = "all (lreg)", type = "scattergl", mode = "lines", line = list(color = adjustcolor("dodgerblue2", alpha.f = 0.95), width = 3, dash = "dash"), hoverinfo = 'text', text = s_text, showlegend = F)
		}
		...
		if (i_num == 1) {
			...
			if (b_cond) {
				ply_1 <- add_trace(p = ply_1, x = v_x, y = v_y, name = "all (lreg)", type = "scattergl", mode = "lines", line = list(color = adjustcolor("dodgerblue2", alpha.f = 0.95), width = 3, dash = "dash"), hoverinfo = 'text', text = s_text, showlegend = F)
			}
