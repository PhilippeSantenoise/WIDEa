...
f_create_stat_method_message <- function(df_stat_method, l_stat_method_level, v_stat_method, o_parameter, df_all) {
	...
	for (i in as.vector(unique(df_stat_method$code))) {
		s_name <- paste(as.vector(unique(df_stat_method[which(df_stat_method$code == i), "message"])), collapse = ", ")
		
		if (length(l_stat_method_level[[i]]) == 0) { # error message
			v_type <- c(v_type, "error")
			
			if (i == "cp1") {
				v_message <- c(v_message, paste0("Error with statistical method (", s_name, "): insufficient size of data (< 3) or the standard deviation of ", ifelse(shiny::isolate(o_parameter$model) == "none", "X" , "fit"), " and/or Y variable(s) is equal to 0", ifelse(!is.na(shiny::isolate(o_parameter$group)), " for each group", "")))
			}
			else { # cp2
				v_message <- c(v_message, paste0("Error with statistical method (", s_name, "): insufficient size of data (< 2) or the standard deviation of X variable is equal to 0", ifelse(!is.na(shiny::isolate(o_parameter$group)), " for each group", "")))
			}
		}
			...
			if (i == "cp1") {
				v_message <- c(v_message, paste0("Error with statistical method (", s_name, "): insufficient size of data (< 3) or the standard deviation of ", ifelse(shiny::isolate(o_parameter$model) == "none", "X" , "fit"), " and/or Y variable(s) is equal to 0 for one or more groups (", paste(v_group[v_pos], collapse = ", "), ")"))
			}
			else { # cp2
				v_message <- c(v_message, paste0("Error with statistical method (", s_name, "): insufficient size of data (< 2) or the standard deviation of X variable is equal to 0 for one or more groups (", paste(v_group[v_pos], collapse = ", "), ")"))
			}
		}
	}
