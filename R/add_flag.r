...
#' Adding flag data in the main plotly graph

#' @description
#' `f_add_flag` returns a plotly object corresponding to flag data information. Flag
#' data are separated into two categories:  
#' \cr(1) Previous flags are data saved in the e_data environment (e_data$flag) and 
#' splitted by qc value in the o_plot reactive value (o_plot$data_qc1,
#' o_plot$data_qc2);
#' \cr(2) Current flags are data saved in the e_current_flag environment.
...
#' @param v_cond is a vector with binary values corresponding to 3 elements (flag,
#' qc1 and qc2). A zero value means no (qc1/qc2) flag data exist. If flag = 1, then
#' qc1 = 1 and/or qc2 = 1 (only temporal/ir data type).
...
#' @param df_previous_flag are flag data saved in the e_data environment (first
#' category). This input is only used for the ir data type (default: NULL).
#' @param e_current_flag is the environment corresponding to flag data of the second
#' category.
...
f_add_flag <- function(s_data_type = "normal", ply_1, v_cond = rep(0, 3), df_click_legend, o_plot, o_parameter, df_previous_flag = NULL, e_current_flag) {
	...
	if (s_data_type == "normal") {
			...
			if (isolate(o_parameter$model) == "none") {
				if (isolate(o_parameter$dim_num) == "2d") { # 2D plot
					...
					if (v_cond[3] == 1) {
						...
						ply_1 <- add_trace(p = ply_1, x = df_qc2[, isolate(o_parameter$x)], y = df_qc2[, isolate(o_parameter$y)], z = df_qc2[, isolate(o_parameter$z)], type = "scatter3d", mode = "markers", name = "qc = 2", marker = list(symbol = "circle-open", line = list(width = 2), color = adjustcolor("red", alpha.f = 1), size = 8), hoverinfo = 'text', text = v_text, showlegend = T, visible = v_visible)
					}
					...
					if (length(e_current_flag) > 0) {
						...
						eval(parse(text = paste(paste0("ply_1 <- add_trace(p = ply_1, x = ", df_all[, isolate(o_parameter$x)], ", y = ", df_all[, isolate(o_parameter$y)], ", z = ", df_all[, isolate(o_parameter$z)], ", type = \"scatter3d\", mode = \"markers\", marker = list(symbol = \"circle-open\", line = list(width = 2), color = adjustcolor(\"black\", alpha.f = 1), size = 8), hoverinfo = \"text\", text = \"", v_text, "\", showlegend = F)"), collapse = "; ")))
					}
				}
			}
		}
		...
		if ("coord" %in% ls(e_current_flag)) {
			...
			v_type <- eval(parse(text = paste0("c(", paste(paste0("l_command[[", 1:length(l_command), "]][[4]]"), collapse = ", "), ")"))) 
