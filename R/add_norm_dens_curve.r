...
f_add_norm_dens_curve <- function (ply_1, v_group = "all", df_click_legend, o_plot, o_parameter, l_graph_opt) {
	...
	if (!is.na(isolate(o_parameter$group))) {
		...
		eval(parse(text = paste(paste0("ply_1 <- add_trace(p = ply_1, x = l_val[[", v_order, "]][[1]], y = l_val[[", v_order, "]][[2]], name = \"", v_group[v_order], " (normal curve)\", type = \"scatter\", mode = \"lines\", line = list(color = adjustcolor(\"", v_subcolor[v_order], "\", alpha.f = 1), dash = \"dash\"), hoverlabel = list(bgcolor = adjustcolor(\"", v_subcolor[v_order], "\", alpha.f = 1)), showlegend = T, visible = ", v_visible[v_order], ")"), collapse = "; ")))
	}
	else {
		...
		ply_1 <- add_trace(p = ply_1, x = v_x_norm, y = v_y_norm, name = "all (normal curve)", type = "scatter", mode = "lines", line = list(color = adjustcolor(l_graph_opt$color, alpha.f = 1), dash = "dash"), hoverlabel = list(bgcolor = adjustcolor(l_graph_opt$color, alpha.f = 1)), showlegend = T, visible = eval(parse(text = v_visible)))
	}
